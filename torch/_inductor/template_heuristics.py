...
class BaseConfigHeuristic(metaclass=BaseHeuristicSingleton):
    ...
    # Flex attn helpers
    def get_flex_attn_fwd_configs(self, head_dim, dtype):
    ...
    def get_flex_attn_bwd_configs(self, head_dim, dtype):
    ...
    def get_flex_decode_configs(self, head_dim, dtype):
...
class CUDAConfigHeuristic(BaseConfigHeuristic):
    ...
    def get_flex_attn_fwd_configs(self, head_dim, dtype):
        ...
        if capability >= (9, 0):
            default_config = self.h100_default_flex_config.get((dtype, head_dim), default_config)
        elif capability >= (8, 0):
...
class ROCmConfigHeuristic(BaseConfigHeuristic):
    ...
    def get_flex_attn_fwd_configs(self, head_dim, dtype):
        ...
        if head_dim <= 256:
            ...
            default_config = self.default_flex_config.get((dtype, head_dim), default_config)
        else:
            if dtype == torch.float32:
